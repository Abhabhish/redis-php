...
$state_url    = trim(str_replace([' ','_'], ['-','-'], strtolower($state_url)));
$district_url = trim(str_replace([' ','_'], ['-','-'], strtolower($district_url)));
$location_url = trim(str_replace([' ','_'], ['-','-'], strtolower($location_url)));
...
if ($location_url != '' || $location_url != null) {
    ...
    // 📦 Cache key for primary location pin query
    $pin_cache_key = "pinquery:$state_url:$district_url:$location_url";

    if ($redis && $redis->exists($pin_cache_key)) {
        error_log("✅ Redis HIT: $pin_cache_key");
        $rowpin = json_decode($redis->get($pin_cache_key), true);
    } else {
        error_log("❌ Redis MISS: $pin_cache_key — querying DB");
        $sql_q = "SELECT p.pin, p.location, s.state, d.district 
                  FROM pincode p 
                  INNER JOIN district_location dl ON dl.lid = p.id 
                  INNER JOIN districts d ON d.did = dl.did 
                  INNER JOIN state_district sd ON d.did = sd.did 
                  INNER JOIN states s ON s.id = sd.sid 
                  WHERE p.search_location = '$location_url' 
                  AND d.search_district = '$district_url' 
                  AND s.surl = '$state_url'";

        $query2 = mysqli_query($db, $sql_q);

        if (mysqli_num_rows($query2) > 0) {
            $rowpin = mysqli_fetch_assoc($query2);
            if ($redis) $redis->setex($pin_cache_key, 21600, json_encode($rowpin)); // 6-hour cache
        } else {
    ...
    if (isset($pin)) {
        ...
        $output .= '<table>';
        $output .= '<tr><th colspan="4" style="text-align:center"><h2>'.$location.' Pincode Details</h2></th></tr>';    
        $output .= '<tr>';
        $output .= '<td><b>Location</b></td>';
        $output .= '<td><b>Pincode</b></td>';
        $output .= '<td><b>State</b></td>';
        $output .= '<td><b>District</b></td>';
        $output .= '</tr>';
        $output .= '<tr>';
        $output .= '<td>'.$location.'</td>';
        $output .= '<td><b>'.$pin.'</b></td>';
        $output .= '<td>'.$state.'</td>';
        $output .= '<td>'.$district.'</td>';
        $output .= '</tr>';            
        $output .= '</table>';

        // 🌐 SEO content (still dynamically generated)
        ...
        // 📦 Cache key for nearby pin locations
        $nearby_cache_key = "nearby:$pin:$location";

        if ($redis && $redis->exists($nearby_cache_key)) {
            $nearby_results = json_decode($redis->get($nearby_cache_key), true);
        } else {
            $nearby_results = [];
            ...
            while ($rslt_pinquery = mysqli_fetch_array($matching_pinquery)) {
                $nearby_results[] = $rslt_pinquery;
            }
            if ($redis) $redis->setex($nearby_cache_key, 21600, json_encode($nearby_results));
        }

        if (!empty($nearby_results)) {
            $output .= '<br /><br /> 
            ...
            foreach ($nearby_results as $row) {
                $mloc    = $row['location'];
                $ms_loc  = $row['search_location'];
                $lid     = $row['id'];

                // Cache keys per location-district-state
                $dkey = "dl:$lid";
                $did = $redis && $redis->exists($dkey) ? $redis->get($dkey) : null;

                if (!$did) {
                    $res = mysqli_query($db, "SELECT did FROM district_location WHERE lid = $lid");
                    $did = mysqli_fetch_row($res)[0];
                    if ($redis) $redis->setex($dkey, 21600, $did);
                }

                $dis_key = "district:$did";
                $districtRow = $redis && $redis->exists($dis_key) ? json_decode($redis->get($dis_key), true) : null;
                if (!$districtRow) {
                    $res = mysqli_query($db, "SELECT district, search_district FROM districts WHERE did = $did");
                    $districtRow = mysqli_fetch_row($res);
                    if ($redis) $redis->setex($dis_key, 21600, json_encode($districtRow));
                }

                $mdis   = $districtRow[0];
                $ms_dis = $districtRow[1];

                $sid_key = "sid:$did";
                $sid = $redis && $redis->exists($sid_key) ? $redis->get($sid_key) : null;
                if (!$sid) {
                    $res = mysqli_query($db, "SELECT sid FROM state_district WHERE did = $did");
                    $sid = mysqli_fetch_row($res)[0];
                    if ($redis) $redis->setex($sid_key, 21600, $sid);
                }

                $state_key = "state:$sid";
                $stateRow = $redis && $redis->exists($state_key) ? json_decode($redis->get($state_key), true) : null;
                if (!$stateRow) {
                    $res = mysqli_query($db, "SELECT state, surl FROM states WHERE id = $sid");
                    $stateRow = mysqli_fetch_row($res);
                    if ($redis) $redis->setex($state_key, 21600, json_encode($stateRow));
                }
...
function strProper($str) {
    ...
    for($i=1; $i<strlen($str)-1; ++$i) {
        if($str[$i]==' ' || $str[$i]=='.') {
            for($j=$i+1; $j<strlen($str) && ($str[$j]!=' ' || $str[$j]!='.'); ++$j);
            $size = $j-$i-1;
            $shortWord = false;
            if($size<=3) {
