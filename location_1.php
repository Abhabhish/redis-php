...
try {
    ...
    $redis->auth('foobared'); // Replace 'foobared' with your actual password
} catch (Exception $e) {
    error_log("Redis connection failed: " . $e->getMessage());
    $redis = null; // Fall back to no caching if Redis is down
}

// 📥 Get and sanitize input from URL
...
// 🗝️ Build Redis cache key
$cache_key = "pincode:india:$state_url:$district_url:$location_url";
...
if ($redis) {
    ...
    if ($cached_output) {
        echo $cached_output;
        exit(); // Serve cached content and stop further processing
    }
}
...
if ($location_url != '' || $location_url != null) {
    ...
    if (isset($pin)) {
        ...
        // 🔍 SEO meta values (used elsewhere in the HTML output probably)
        ...
        // 📍 Nearby locations with same PIN
    ...
    // 💾 Save to Redis cache if output is valid
    if (!$flagOutput && $redis) {
        $redis->setex($cache_key, 21600, $output); // Cache for 6 hours
    }
